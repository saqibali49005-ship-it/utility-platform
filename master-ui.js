...
function initHeader(){
 ...
 if(n){
  ...
  n.classList.remove('is-open');
   ...
   if(document.body.classList.contains('tool-page')&&m&&n){m.onclick=function(e){e.preventDefault();e.stopPropagation();const open=!n.classList.contains('is-open');n.classList.toggle('is-open',open);n.style.setProperty('display',open?'grid':'none','important');m.setAttribute('aria-expanded',String(open));m.setAttribute('aria-label',open?'Close menu':'Open menu');if(open&&p){p.style.setProperty('display','none','important');s&&s.setAttribute('aria-expanded','false');}};}\nif(s&&p){s.onclick=function(e){e.preventDefault();e.stopPropagation();const open=p.style.display!=='block';p.style.setProperty('display',open?'block':'none','important');s.setAttribute('aria-expanded',String(open));if(open&&i)i.focus();if(open&&n){n.classList.remove('is-open');m&&m.setAttribute('aria-expanded','false');m&&m.setAttribute('aria-label','Open menu');}};}
}
initHeader();
function initSearch(){
 if(!i||!f)return;
